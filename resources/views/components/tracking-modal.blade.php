...
<div id="trackingModal" style="display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:1000;padding:20px;overflow-y:auto;">
    <div style="background:#FFFFFF;border-radius:12px;max-width:90%;width:90%;max-height:90vh;margin:20px auto;box-shadow:0 20px 60px rgba(0,0,0,0.15);animation:slideUp 0.3s ease-out;display:flex;flex-direction:column;">
        ...
        <!-- Progress Timeline (Status Flow Details) -->
        <div style="padding:40px 24px;border-bottom:1px solid #E5E5E5;background:#FFFFFF;">
            <div id="progressTimeline" style="display:flex;gap:24px;justify-content:center;align-items:flex-start;">
...
function generateProgressTimeline(currentStatus, tracking) {
    ...
    const allStatuses = paths[currentStatus] || paths.scheduled;
    ...
    // Create status details cards
    const statusHTML = allStatuses.map((status, index) => {
        const isCompleted = index < allStatuses.length - 1 || (index === allStatuses.length - 1 && currentStatus !== 'scheduled');
        const isCurrent = allStatuses[index] === statusLabels[currentStatus];

        let statusColor = '#CCCCCC';
        if (isCompleted || isCurrent) {
            if (status === 'Completed') statusColor = '#00AA00';
            else if (status === 'Cancelled' || status === 'No Show') statusColor = '#CC0000';
            else statusColor = '#000000';
        }
        ...
        return `
            <div style="
                ...
                text-align: center;
                padding: 24px;
                background: ${isCompleted || isCurrent ? '#F8F9FA' : '#FFFFFF'};
